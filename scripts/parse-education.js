...
for (let line of lines) {
  ...
  if (
    ...
    (trimmedLine.startsWith("<p>") && trimmedLine.includes("<strong><u>"))
  ) {
    saveCurrentSubsection();
    const title = cleanHtmlText(trimmedLine);
    if (title) {
...
const output = {
  ...
  description:
    "A comprehensive guide covering dementia types, caregiving essentials, and self-care for caregivers in Malaysia",
...
sections.forEach((section, idx) => {
  console.log(
    `  ${idx + 1}. ${section.title} (${section.subsections.length} subsections)`,
  );
});
